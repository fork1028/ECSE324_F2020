...
IDLE:
	LDR R5, =PB_int_flag
	LDR R6, [R5]
	CMP R6, #1 // START is released
	MOVEQ R9, #1 // start the timer
	CMP R6, #2 // STOP is released
	MOVEQ R9, #0 // stop the timer
	CMP R6, #4 // RESET is released
	MOVEQ R9, #0
	MOVEQ R7, #0
	MOVEQ R8, #0
	MOVEQ R12, #0
...
KEY_ISR:
    PUSH {LR}
	BL read_PB_edgecp_ASM		// Get pushbutton that was pressed

	LDR R1, =PB_int_flag
	STR R0, [R1]				// Set flag to value of pb

	BL PB_clear_edgecp_ASM		// Clear Edgecap to reset interrupt
